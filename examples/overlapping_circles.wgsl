...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    for (var i: u32 = 1u; i < 500u; i = i + 1u) {
        ...
        let xx = fract(rx + sx * uniforms.time / 18.) * 2.4 - 1.2;
        let yy = fract(ry + sy * uniforms.time / 18.) * 2.4 - 1.2;
    ...
    if (c >= 1u && abs(in.coord.y - uniforms.mouse.y) < 0.3 && uniforms.mouse.y < 0.999) {
        c = 1u - c;
    }
